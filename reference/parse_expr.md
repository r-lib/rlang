...
- `parse_quo()` and `parse_quos()` are variants that create a
  [quosure](https://rlang.r-lib.org/reference/defusing-advanced.md).
  ...
  [`enquos()`](https://rlang.r-lib.org/reference/enquo.md), or `{{`, a
  parsed quosure never contains injected quosures. It is thus safe to
...
- env:
  ...
  environment](https://rlang.r-lib.org/reference/search_envs.md) (the
  default) may be the right choice when you are parsing external user
  inputs. You might also want to evaluate the R code in an isolated
  context (perhaps a child of the global environment or of the [base
  environment](https://rlang.r-lib.org/reference/search_envs.md)).

## Value

`parse_expr()` returns an
[expression](https://rlang.r-lib.org/reference/is_expression.md),
