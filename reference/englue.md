...
operators](https://rlang.r-lib.org/reference/glue-operators.md) `{` and
`{{`. These operators are normally used to inject names within [dynamic
dots](https://rlang.r-lib.org/reference/dyn-dots.md). `englue()` makes
them available anywhere within a function.
...
[defuses](https://rlang.r-lib.org/reference/topic-defuse.md) `arg` and
transforms the expression to a string with
