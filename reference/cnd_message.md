...
[`abort()`](https://rlang.r-lib.org/reference/abort.md) only need to
...
[lambda-formula](https://rlang.r-lib.org/reference/as_function.md), or a
function with the same signature as `cnd_header()`, `cnd_body()`, or
`cnd_footer()`. These fields override the message generics and make it
easy to generate an error message tailored to the state in which the
