...
[defuse](https://rlang.r-lib.org/reference/topic-defuse.md) R
expressions. [`expr()`](https://rlang.r-lib.org/reference/expr.md),
[`enquo()`](https://rlang.r-lib.org/reference/enquo.md), and
[`enquos()`](https://rlang.r-lib.org/reference/enquo.md) are sufficient
for most purposes but rlang provides these other operations, either for
completeness or because they are useful to experts.

- `exprs()` is the plural variant of
  [`expr()`](https://rlang.r-lib.org/reference/expr.md). It returns a
  list of expressions. It is like
  [`base::alist()`](https://rdrr.io/r/base/list.html) but with
  [injection](https://rlang.r-lib.org/reference/topic-inject.md)
  support.

- `quo()` and `quos()` are like
  [`expr()`](https://rlang.r-lib.org/reference/expr.md) and `exprs()`
  but return quosures instead of naked expressions. When you are
  defusing your own local expressions (by opposition to function
...
- `enexpr()` and `enexprs()` are like
  [`enquo()`](https://rlang.r-lib.org/reference/enquo.md) and
  [`enquos()`](https://rlang.r-lib.org/reference/enquo.md) but return
  naked expressions instead of quosures. These operators should very
  rarely be used because they lose track of the environment of defused
  arguments.
...
- `enquo0` and `enquos0()` are like
  [`enquo()`](https://rlang.r-lib.org/reference/enquo.md) and
  [`enquos()`](https://rlang.r-lib.org/reference/enquo.md) but without
  injection support. The injection operators `!!`, `!!!`, and `{{` are
  not processed, instead they are preserved in the defused expression.
  This makes it possible to defuse expressions that potentially contain
  injection operators meant for later use. The trade off is that it
  makes it harder for users to inject expressions in your function. They
  have to enable injection explicitly with
  [`inject()`](https://rlang.r-lib.org/reference/inject.md).

  None of the features of [dynamic
  dots](https://rlang.r-lib.org/reference/dyn-dots.md) are available
...
- ...:
  ...
  [`enquos()`](https://rlang.r-lib.org/reference/enquo.md), names of
