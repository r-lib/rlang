...
r_obj* ffi_try_fetch(r_obj* try_fetch_args) {
  ...
  for (int i = n - 1; i >= 0; --i) {
    ...
    if (cls == r_strs.error) {
      r_obj* exiting_hnd =
          KEEP(r_call3(r_syms.brackets2, rlang_syms.handlers, r_int(i + 1)));
    ...
    // Picks up `I`
    r_obj* subscript_node =
        r_node_cddr(r_node_caar(r_node_cddr(r_node_cadr(r_node_cadr(r_node_cdar(
