...
r_obj* ffi_format_error_arg(r_obj* arg) {
  switch (r_typeof(arg)) {
  case R_TYPE_symbol:
    arg = r_sym_as_utf8_character(arg);
    break;
  case R_TYPE_string:
    arg = r_str_as_character(arg);
    break;
  case R_TYPE_call:
    arg = r_as_label(arg);
    break;
...
r_no_return void rlang_stop_internal2(
    const char* file,
    int line,
    r_obj* call,
    const char* fmt,
...
r_no_return void rlang_stop_internal(const char* fn, const char* fmt, ...) {
  R_CheckStack2(BUFSIZE);
  char msg[BUFSIZE];
  INTERP(msg, fmt, ...);
...
static r_no_return r_obj* stop_internal_cb(void* payload) {
...
static r_obj* with_winch(void* payload) {
...
static void without_winch(void* payload) {
...
const char* rlang_obj_type_friendly_full(r_obj* x, bool value, bool _length) {
  r_obj* out_obj =
      KEEP(r_eval_with_xy(obj_type_friendly_call, x, r_lgl(value), rlang_ns_env)
      );
...
static r_obj* format_arg_call = NULL;

static r_obj* obj_type_friendly_call = NULL;
