...
* New type-checking functions exported from rlang: `check_bool()`,
  ...
  `check_number_whole()`, `check_symbol()`, `check_call()`,
  `check_environment()`, `check_function()`, `check_closure()`,
  `check_formula()`, and `check_data_frame()`. These were previously
  only available via the `standalone-types-check.R` standalone file.
  `stop_input_type()` is also now exported.
