#' Capture an expression with interpolation.
#'
#' These functions return their argument as an unevaluated
#' expression. Interpolation is applied  Interpolation replaces sub-expressions of the form
#' \code{UQ(x)} with the evaluated value of \code{x}, and inlines
#' sub-expressions of the form \code{UQS(x)}. Syntactic shortcuts are
#' provided for unquoting and unquote-splicing by prefixing with
#' \code{!!} and \code{!!!}.
#'
...
#'
#' @param expr An expression that will be captured unevaluated and
#'   interpolated.
#' @param x For \code{UQ()} and \code{UQF()}, a formula. For
#'   \code{UQS()}, a a vector.
...
#' f_quote(x ~ 1 +  !! (1 + 2 + 3) + 10)
#' f_quote(x ~ 1 + (!! (1 + 2 + 3)) + 10)
#'
#'
#' # When a formula is unquoted, f_quote() checks whether its
...
#' var <- disp ~ am
#' f_quote(lm(!!var, mtcars))
#' f_quote(lm(UQF(var), mtcars))
#'
#'
