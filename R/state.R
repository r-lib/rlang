...
#' Cat a line with a trailing newline
#'
#' @description
#'
#' This little helper is like `cat()` with a couple of differences:
#'
#' * Instead of interleaving its inputs with spaces, it interleaves
#'   new lines.
#'
#' * It automatically adds a new line at the end.
#'
#' `meow()` always outputs to the console while `conn_meow()` takes a
#' connection to output to.
#'
#' @param ... Output lines. These are passed to [chr()] so lists of
#'   character vectors are automatically flattened.
#' @param .conn A connection to output to.
#' @export
#' @examples
#' # meow() supports lists of character vectors:
#' lines <- list(
#'   "<my-type>",
#'   "Metadata: foo"
#' )
#'
#' # It automatically adds newlines to all its inputs:
#' meow(lines)
#' print(letters)
#' meow(
#'   "---",
#'   paste("n:", length(letters)),
#'   paste("type:", typeof(letters))
#' )
meow <- function(...) {
...
#' @rdname meow
#' @export
conn_meow <- function(.conn, ...) {
  cat(chr_lines(..., .trailing = TRUE), file = .conn)
}
