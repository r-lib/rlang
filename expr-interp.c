...
struct expansion_info which_bang_op(sexp* first) {
  ...
  // FIXME: what if we've got an OP_EXPAND_FIXUP?
  // probably just reassign CAR or something
  if (r_is_call(first, "(")) {
...
// FIXME: which_bang_op() might poke around?
...
static sexp* expr_interp(sexp* x, sexp* env) {
  ...
  if (expr_maybe_needs_fixup(x)) {
    ...
    if (rotation_info.pivot) {
      r_printf("rotate outer\n");
