...
Lazy evaluation is a principled way to do non-standard evaluation (NSE) in R. It is centered around formulas, which capture an expression and the current environment. You use lazy evaluation by requiring the user to "quote" specially evaluated arguments with `~`, and then using the lazyeval package to compute on those formulas. It is also possible to eliminate the use of the `~` by converting promises to formulas. This does make programming with such functions a little harder, but it can be worth it in certain situations.

You'll find vignette helpful if you want to program with functions from my packages (like the development versions of dplyr and ggplot2), or you want a principled way of working with delayed expressions in your own package.
...
* To delay the computation of an expression you must capture both the 
  ...
  explicit way of doing this; lazyeval uses C to extract the expression
  and environment for any promise.
...
* R lacks a built-in quasiquotation mechanism which allows you to "unquote"
  expression in order to mix evaluated and unevaluated components in a single
  object. Lazyeval provides `finterp()` which implements a full quasi-quotation
  tool with both unquote and unquote-splice tools.
...
In this sense, the formula operator is a general "quoting" operator for R. The formula allows you to capture the essence of a computation: both the sequence of operations and the environment in which those computations need to be carried out.

```{r}
# The 1st element is always ~
f[[1]]
# The 2nd element is the call itself
f[[2]]
```
...
To avoid this problem, we'll only ever use the single-sided formula for quoting, and use `rhs()` to access the call.
...
The lazyeval package provides a number of functions to make working with formulas easier. For example, the `feval()` function evaluates the formula in its environment.
...
This is powerful because it allows us to separate the definition of a computation from its evaluation. This is useful because we can compute on the expression itself, or as we'll see shortly, evaluate it in a slightly non-standard way. 

The formula captures the environment in which it was evaluated, so `feval()` works even when the formula is used in a different place from where it was defined.
