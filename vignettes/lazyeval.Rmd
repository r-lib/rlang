...
A more general solution is to allow the user to supply a formula, and use the special `uq()` function. When used with `f_eval()`, anything inside `uq()` is replaced with its evaluated value. 
...
1.  `feval()` allows "unquote" and interpolates in evaluated vlaues inside 
    `uq()`. It's easiest to see what's happening by using the `f_interp()` 
