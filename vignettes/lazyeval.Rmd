...
As the name suggests, non-standard evaluation (NSE) breaks away from the standard evaluation (SE) rules in order to do something special. There are three common uses of NSE:
...
1.  Implement non-standard _scoping_, typically looking for values in a data 
    ...
    transform(df, z = x + y)
    ```
1.  The handful of other uses (such as in `bquote()` and `library()`) are often 
    called [metaprogramming](http://adv-r.had.co.nz/Expressions.html) because 
    they all involve computing on the unevaluated code in some way.

I'll briefly discuss labelling because it's hard to do 100% safely with base R tools, and lazyeval provides some useful helpers. The rest of the vignette will focus on non-standard scoping because it's the most commonly used, most important, and hardest to do correctly. 

## Labelling

In base R, the classic way to turn an argument into a string is to use `deparse(substitute(x))`:
...
There are two things you need to be aware of with this approach:

1.  For long expressions, `deparse()` generates a character vector with 
    length > 1.
...
Both of these problems are resolved by `expr_text()`:

```{r}
foo <- function(x) expr_text(x)
bar <- function(x) foo(x)
...
Lazyeval also provides `expr_find()` to find the underlying expression - it works similarly to `substitute()` but will follow a chain of promsies back up to the original expression. `expr_label()` is a customised version of `expr_label()` that produces labels designed to be used in messages to the user:
...
1.  To implememt non-standard scoping, you must delay the evaluation of a 
    function argument, capturing the computation (the expression), and where
    the computation should occur (the environment). Base R formulas, described 
    in detail next, provide a convenient and explicit way to do this. 
  
1.  Once you have a formual, a delayed expression, you need some way to 
    evaluate it. Lazyeval provides the `f_eval()` function which adds some
    useful tricks to `base::eval()` in order to make it safer to program with.

1.  You often want to mix evaluated and unevaluated components in a 
    single call. `f_eval()` implements a full __quasi-quotation__ system which
    allows you to "unquote" (i.e. evaluate) expression inside a formula.

## Formulas

Formulas are a familiar tool from linear models, but they are actually a powerful general purpose tool. A formula captures two things: 

1. An unevaluated expression
1. The environment in which the expression was created

`~` is a single character that allows you to say: "I want capture the meaning of this code, without evaluating it right away". For that reason, the formula can be thought of as general "quoting" operator.
...
To avoid these differences,  lazyeval provides `f_rhs()` and `f_lhs()` to reliably access either side of a formula:
...
A formula captures the meaning of an expression without evaluating it. You can force evaluation of a formula with `f_eval()`:
...
This allows you to use a formula a way of delaying evaluation. Because the formula captures both expression and environment `f_eval()` is robust, and works even when the formula is used in a different place from where it was defined:
...
This code is fairly simple. We require the user to provide a formula and then evaluate it with `f_eval()`. Once we've checked that we have a logical vector, it only remains to replace `NA` with `FALSE`, and to perform the subsetting.
