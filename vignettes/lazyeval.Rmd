...
This document describes lazyeval, a package that provides tools to perform non-standard evaluation (NSE) in R. As the name suggests, non-standard evaluation breaks away from the standard evaluation (SE) rules in order to do something special. There are three common uses of NSE:

1.  __Labelling__: it is often useful to generate labels based on the expression 
    used to generate the value for a function argument. For example, note 
    the x and y axis labels in this plot:
...
1.  __Non-standard scoping__ looks for objects in places other than the current
    ...
    `transform()` that look for variables in a data frame (or list) before the 
    parent environment:
...
The lazyeval package provides principled tools to help you correctly implement these techniques in your own code. You should read this vignette if you want to program with packages like dplyr and ggplot2[^1], or you want a principled way of working with delayed expressions in your own package.
...
This allows you to use a formula as a robust way of delaying evaluation. Note that this works even when the formula is used in a different place from where it was defined. In the following example, note that the value of `x` inside `add_1000` is used, not the value of `x` at the top level.
...
`f_eval()` has an optional second argument: a list (or data frame) that overrides values found in the environment. This makes it very easy to implement non-standard scoping:

```{r}
f_eval(~ mean(cyl), data = mtcars)
```

One challenge with non-standard scoping is that we've introduced some ambiguity. For example, when we refer to `x` in the code below, does it come the environment of the formula or from `my_data`? You can't know without knowing whether or not `mydata` has a variable called `x`.

```{r, eval = FALSE}
f_eval(~ x, data = mydata)
```

To overcome this problem, `f_eval` provides two pronouns:
...
```{r}
mydata <- data.frame(x = 100, y = 1, z = 3)
x <- 10
f_eval(~ x, data = mydata)
...
Note that if the variable/object doesn't exist, you get an informative error:

```{r, error = TRUE}
f_eval(~ .env$y, data = mydata)
f_eval(~ .data$y, data = mydata)
```

### Unquoting

`f_eval()` has one more useful trick up its sleeve: unquoting. This allows you to write functions where the user can supply some component of the formula. For example, the following function allows you to compute the mean of any column (or any function of a column):
...
To see how this works, we can use `f_interp()` which `f_eval()` calls internally (you shouldn't call it in your own code, but it's useful for debugging). `uq()` evaluates it's first argument and inserts its value into the formula:
...
To illustrate these challenges, I'm going to implement a `sieve()` function that works similarly to `base::subset()` or `dplyr::filter()`. The goal of `sieve()` is to make it easy to select observations matching criteria defined by values of the variables. `sieve()` has three advantages over `[`:
...
The implementation of `sieve()` is straightforward. We use `f_eval()` to perform NSS. Then we then check that we have a logical vector, replace `NA`s with `FALSE`, and subset.
