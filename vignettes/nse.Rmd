...
Both of these problems are resolved by `rlang::expr_text()`:
...
Then create the NSE version which doesn't need the explicit formula. The key is the use of `tidy_capture()` which takes an unevaluated argument (a promise) and captures it as a formula:

```{r}
sieve <- function(df, expr) {
  sieve_(df, tidy_capture(expr))
}
sieve(df, x == 1)
```

If you're familiar with `substitute()` you might expect the same drawbacks to apply. However, `tidy_capture()` is smart enough to follow a chain of promises back to the original value, so, for example, this code works fine:
...
If you want a `...` function that doesn't require formulas, I recommend that the SE version take a list of arguments, and the NSE version uses `tidy_dots()` to capture multiple arguments as a list of formulas.
