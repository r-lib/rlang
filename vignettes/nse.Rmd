...
Both of these problems are resolved by `rlang::arg_text()`:

```{r}
my_label <- function(x) arg_text(x)
my_label2 <- function(x) my_label(x)
...
There are two variations on the theme of `arg_text()`:

*   `arg_expr()` find the underlying expression. It works similarly to
...
*   `arg_label()` is a customised version of `arg_text()` that produces
...
1.  `plot()` uses `deparse(substitute(x))` to generate labels for the x and y
    ...
    Write your own wrapper around `plot()` that uses `arg_label()` to compute
    `xlim` and `ylim`.
...
1.  Read the source code for `arg_text()`. How does it work? What additional
...
To abstract away these differences, rlang provides `f_rhs()` and `f_lhs()` to access either side of the formula, and `f_env()` to access its environment:
...
A formula captures delays the evaluation of an expression so you can later evaluate it with `tidy_eval()`:
...
`tidy_eval()` has an optional second argument: a named list (or data frame) that overrides values found in the formula's environment.

```{r}
y <- 100
tidy_eval(~ y)
tidy_eval(~ y, data = list(y = 10))

# Can mix variables in environment and data argument
tidy_eval(~ x + y, data = list(x = 10))
# Can even supply functions
tidy_eval(~ f(y), data = list(f = function(x) x * 3))
```

This makes it very easy to implement non-standard scoping:

```{r}
tidy_eval(~ mean(cyl), data = mtcars)
```
...
```{r, eval = FALSE}
tidy_eval(~ x, data = mydata)
```

You can't tell without knowing whether or not `mydata` has a variable called `x`. To overcome this problem, `tidy_eval()` provides two pronouns:
...
tidy_eval(~ .env$x, data = mydata)
tidy_eval(~ .data$x, data = mydata)
```

If the variable or object doesn't exist, you'll get an informative error:

```{r, error = TRUE}
tidy_eval(~ .env$z, data = mydata)
tidy_eval(~ .data$z, data = mydata)
```

### Unquoting

`tidy_eval()` has one more useful trick up its sleeve: unquoting. Unquoting allows you to write functions where the user supplies part of the formula. For example, the following function allows you to compute the mean of any column (or any function of a column):

```{r}
df_mean <- function(df, variable) {
  tidy_eval(~ mean(UQ(variable)), data = df)
}
...
To see how this works, we can use `tidy_interp()` which `tidy_eval()` calls internally (you shouldn't call it in your own code, but it's useful for debugging). The key is `UQ()`: `UQ()` evaluates its first (and only) argument and inserts the value into the formula:

```{r}
variable <- ~cyl
tidy_interp(~ mean(UQ(variable)))
...
tidy_interp(~ mean(UQ(variable), UQS(extra_args)))
```
...
1.  Compare and contrast `tidy_eval()` with `with()`.
...
1.  You must correctly delay the evaluation of a function argument, capturing
    ...
    arguments with `~`, and then evaluating explicit with `tidy_eval()`.
...
1.  When writing functions that use NSS-functions, you need some way to
    ...
    found. `tidy_eval()` solves this problem with the `.data.` and `.env`
    pronouns.

1.  You need some way to allow the user to supply parts of a formula.
    `tidy_eval()` solves this with unquoting.
...
The implementation of `sieve()` is straightforward. First we use `tidy_eval()` to perform NSS. Then we then check that we have a logical vector, replace `NA`s with `FALSE`, and subset with `[`.

```{R}
sieve <- function(df, condition) {
  rows <- tidy_eval(condition, df)
  if (!is.logical(rows)) {
...
These failures are partiuclarly pernicious because instead of throwing an error they silently produce the wrong answer. Both failures arise because `tidy_eval()` introduces ambiguity by looking in two places for each name: the supplied data and formula environment.
...
In this case, it's the responsibility of the user to ensure the `variable` is specified unambiguously. `tidy_eval()` is designed so that `.data` and `.env` work even when evaluated inside of `UQ()`:
...
rlang provides the `f_list()` function to make writing this sort of function a little easier. It takes a list of formulas and evaluates the LHS of each formula (if present) to rename the elements:
...
That said, rlang does allow you to eliminate the `~` if you really want to. In this case, I recommend having both a NSE and SE version of the function. The SE version, which takes formuals, should have suffix `_`:
...
Then create the NSE version which doesn't need the explicit formula. The key is the use of `arg_capture()` which takes an unevaluated argument (a promise) and captures it as a formula:

```{r}
sieve <- function(df, expr) {
  sieve_(df, arg_capture(expr))
}
sieve(df, x == 1)
```

If you're familiar with `substitute()` you might expect the same drawbacks to apply. However, `arg_capture()` is smart enough to follow a chain of promises back to the original value, so, for example, this code works fine:
