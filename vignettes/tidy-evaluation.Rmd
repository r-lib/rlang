...
sublanguages in R are modelling specifications with formulas (`lm()`,
...
- R code is first-class. That is, R code can be manipulated like
  ...
  [sym()](http://rlang.tidyverse.org/reference/sym.html),
  [lang()](http://rlang.tidyverse.org/reference/lang.html) and
  [node()](http://rlang.tidyverse.org/reference/pairlist.html) for
  creating such objects). We also call _expressions_ these objects
  containing R code (see [is_expr()](http://rlang.tidyverse.org/reference/is_expr.html)).
...
- Scope is first-class. Scope is the lexical environment that
  ...
  created (see
  [env()](http://rlang.tidyverse.org/reference/env.html)) and
  manipulated as regular objects.

- Finally, functions can capture the expressions that were supplied
  as arguments instead of being passed the value of these
  expressions (see
  [enquo()](http://rlang.tidyverse.org/reference/quosure.html) and
  [enexpr()](http://rlang.tidyverse.org/reference/expr.html)).

To sum up, R functions can capture expressions, manipulate them like
regular objects, and alter the meaning of symbols referenced in these
expressions by changing the scope (the environment) upon
...
sublanguage. The first is to parse the expression manually, the other
is to evaluate the expression in a modified environment.
...
method of dealing with expressions is tedious, rigid and error prone
because you're basically rewriting an interpreter of R code. It is
...
should _overscope_ the dynamic context. The traditional solution to
