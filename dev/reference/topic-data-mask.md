...
[`{{`](https://rlang.r-lib.org/dev/reference/embrace-operator.md) or, in
...
[defusing](https://rlang.r-lib.org/dev/reference/topic-defuse.md) R code
to prevent its immediate evaluation. The defused code is resumed later
on in a context where data frame columns are defined.
...
[pronouns](https://rlang.r-lib.org/dev/reference/dot-data.md) to help
...
- [Argument
  defusal](https://rlang.r-lib.org/dev/reference/topic-defuse.md) with
  [`substitute()`](https://rdrr.io/r/base/substitute.html) (base R) or
  ...
  (rlang). R code is defused so it can be evaluated later on in a
  special environment enriched with data.
...
- The rlang package developed tidy eval in 2017 as the data-masking
  ...
  [implicit
  injection](https://rlang.r-lib.org/dev/reference/topic-inject.md) with
  `!!` and `!!!`, and [data
  pronouns](https://rlang.r-lib.org/dev/reference/dot-data.md).
