...
argument to [`abort()`](https://rlang.r-lib.org/dev/reference/abort.md).
It sets the call (or a value that indicates where to find the call, see
...
- call:
  ...
  - An execution environment, e.g. as returned by
    ...
    The [`sys.call()`](https://rdrr.io/r/base/sys.parent.html) for that
...
By default [`abort()`](https://rlang.r-lib.org/dev/reference/abort.md)
uses the function call of its caller as context in error messages:
...
pass it to [`abort()`](https://rlang.r-lib.org/dev/reference/abort.md).
Here we name this argument `error_call` for consistency with `error_arg`
...
call. [`abort()`](https://rlang.r-lib.org/dev/reference/abort.md) stores
the error call in the `call` condition field which is then used to
generate the "in" part of error messages.
...
[`abort()`](https://rlang.r-lib.org/dev/reference/abort.md) to climb the
...
[`caller_env()`](https://rlang.r-lib.org/dev/reference/stack.md) is fast
