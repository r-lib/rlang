...
older R versions,
[`entrace()`](https://rlang.r-lib.org/dev/reference/entrace.md) is set
as an `option(error = )` handler. The latter method has the disadvantage
that only one handler can be set at a time. This means that you need to
manually switch between
[`entrace()`](https://rlang.r-lib.org/dev/reference/entrace.md) and
other handlers like [`recover()`](https://rdrr.io/r/utils/recover.html).
Also this causes a conflict with IDE handlers (e.g. in RStudio).
