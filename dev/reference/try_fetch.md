...
- **Rethrow conditions**, e.g. using `abort(msg, parent = cnd)`. The
  ...
  [`inform()`](https://rlang.r-lib.org/dev/reference/abort.md) can be
  used to chain conditions. This is typically done to add information to
...
- **Inspect conditions**, for instance to log data about warnings or
  ...
  [`zap()`](https://rlang.r-lib.org/dev/reference/zap.md) sentinel to
...
[`abort()`](https://rlang.r-lib.org/dev/reference/abort.md). This is a
...
[`warn()`](https://rlang.r-lib.org/dev/reference/abort.md). In
particular, downgraded conditions (e.g. from error to warning or from
warning to message) are not matched across parents.
...
returns the [`zap()`](https://rlang.r-lib.org/dev/reference/zap.md)
