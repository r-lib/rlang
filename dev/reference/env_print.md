...
- The [label](https://rlang.r-lib.org/dev/reference/env_name.md) and the
  parent label.

- Whether the environment is
  [locked](https://rlang.r-lib.org/dev/reference/env_lock.md).
...
- The bindings in the environment (up to 20 bindings). They are printed
  ...
  addition [fancy
  bindings](https://rlang.r-lib.org/dev/reference/env_bind.md) (actives
  and promises) are indicated as such.

- Locked bindings get a `[L]` tag

Note that printing a package namespace (see
[`ns_env()`](https://rlang.r-lib.org/dev/reference/ns_env.md)) with
