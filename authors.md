# Authors and Citation

## Authors

- **[Lionel Henry](https://github.com/lionel-)**. Author, maintainer.

- **[Hadley Wickham](https://hadley.nz)**. Author.

- **mikefc**. Copyright holder.  
  Hash implementation based on Mike's xxhashlite

- **Yann Collet**. Copyright holder.  
  Author of the embedded xxHash library

- **[![Posit](https://www.tidyverse.org/posit-logo.svg)](https://www.posit.co)**.
  Copyright holder, funder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/r-lib/rlang/blob/v1.3.0/DESCRIPTION)

Henry L, Wickham H (2026). *rlang: Functions for Base Types and Core R
and 'Tidyverse' Features*. R package version 1.3.0,
<https://rlang.r-lib.org>.

    @Manual{,
      title = {rlang: Functions for Base Types and Core R and 'Tidyverse' Features},
      author = {Lionel Henry and Hadley Wickham},
      year = {2026},
      note = {R package version 1.3.0},
      url = {https://rlang.r-lib.org},
    }
