...
The [injection operators][topic-injection] [`{{`][embrace-operator], [`!!`][injection-operator], and [`!!!`][splice-operator] are an extension of the R syntax developed for tidyverse packages. Because they are not part of base R, they suffer from some limitations. In particular no specific error is thrown when they are used in unexpected places.


## Using `{{` out of context

The embrace operator [`{{`][embrace-operator] is a feature available in [data-masked][topic-data-masking] arguments. If you use it elsewhere, it is interpreted as a double `{` wrapping.
...
However in many cases embracing is done to implement [data-masking.][topic-data-masking] It is likely that the function will be called with data-variables references which R won't be able to resolve properly:
...
The injection operators [`!!`] and [`!!!`] are implemented in [data-masked arguments][topic-data-masking], [dynamic dots][dyn-dots], and within [inject()]. When used in other contexts, they are interpreted by R as double and triple _negations_.
