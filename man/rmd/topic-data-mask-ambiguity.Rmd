...
The [`.data`] pronoun may be used as a name-to-data-mask pattern (see [Programming patterns for data masking][topic-data-mask-programming]):
...
[Injection operators][topic-injection] such as [`!!`][injection-operator] have interesting properties regarding the ambiguity problem. They modify a piece of code early on by injecting objects or other expressions before any data-masking logic comes into play. If you inject the _value_ of a variable, it becomes inlined in the expression. R no longer needs to look up any variable to find the value.
