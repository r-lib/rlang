...
While `glue::glue()` only supports `{`, dynamic dots support both `{` and `{{`. The double brace variant is similar to the embrace operator [`{{`][embrace-operator] available in [data-masked][topic-data-mask] arguments. It [defuses][topic-defuse] a function argument, calls [as_label()] on the expression supplied as argument, and inserts the result in the string.
...
Ideally, using `{{` on regular objects would be an error. However for technical reasons it is not possible to make a distinction between function arguments and ordinary variables. See `r links$topic_embrace_non_args` for more information about this limitation.
