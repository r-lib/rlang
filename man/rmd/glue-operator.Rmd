...
[Dynamic dots][dyn-dots] have built-in support for names interpolation with the [glue package](https://glue.tidyverse.org/).
...
Inside functions, embracing an argument with [`{{`][embrace-operator] inserts the expression supplied as argument in the string.
...
While `glue::glue()` only supports `{`, dynamic dots support both `{` and `{{`. The double brace variant is similar to the embrace operator [`{{`][embrace-operator] available in [data-masked arguments][topic-data-masking]. It [defuses][topic-defusal] a function argument, calls [as_label()] on the expression supplied as argument, and inserts the result in the string.
...
`{{` is only meant for function arguments, and only for inserting the expression supplied as argument. If you need to interpolate a string stored in a variable, use the regular glue operator `{` instead:
...
Ideally, using `{{` on regular objects would be an error. However for technical reasons it is not possible to make a distinction between function arguments and ordinary variables. See [Does \\{\\{ work on regular objects?][topic-embrace-non-args] for more information about this limitation.
