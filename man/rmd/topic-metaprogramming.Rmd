...
If you haven't already, read `r links$topic_data_mask_programming`. This article covers simpler patterns that do not require as much theory to get up to speed.
...
You can symbolise a character vector with [syms()] or [data_syms()]. Simple symbols returned by `syms()` work in a wider variety of cases (e.g. with base functions) but we'll use `data_syms()` as it is more robust (see `r links$topic_data_mask_ambiguity`). It creates calls that subset the [`.data`] pronoun. Compare:
...
See `r links$topic_data_mask_ambiguity` for more information about this problem.
