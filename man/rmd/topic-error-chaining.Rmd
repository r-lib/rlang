...
- Compared to `withCallingHandlers()`, which also preserves the error context, `try_fetch()` is able to catch stack overflow errors on R versions >= 4.2.0 (unreleased at the time of writing).
...
Altough we have created a chained error, the error call of the contextual error is not quite right. It mentions the name of the error helper instead of the name of the user-facing function. Furthermore, the error backtrace is rather unhelpful:
...
It also hides all the error helpers from the backtrace:

```{r}
rlang::last_error()
```

__FIXME__: The simplified backtrace should be:

```r
#> Backtrace:
#>  1. global my_verb(add(1, ""))
#>  9. global add(1, "")
```
