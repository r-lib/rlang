...
As a refresher (see the [programming patterns][topic-data-mask-programming] article), there are two common ways of passing arguments to [data-masking][topic-data-mask] functions. For single arguments, embrace with [`{{`][embrace-operator]:
...
Since `{{` inherits behaviour, the first implementation of `my_pivot_longer()` already allows multiple columns passing:
...
An alternative is to use external defusal of arguments. This is what formula interfaces do. A modelling function takes a formula in a regular argument and the formula defuses the user code:
...
Intuitively, many programmers who want to take a list of expressions in a single argument try to defuse an argument and parse it. If the user has supplied argument within a `list()` call, the arguments are taken apart and spliced with `!!!`. Otherwise, the user is assumed to have supplied a single argument which is injected. An implementation along these lines might look like this: 
