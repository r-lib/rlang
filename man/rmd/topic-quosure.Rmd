...
As a rule of thumb, quosures are only needed for arguments defused with [enquo()] or [enquos()] (or with [`{{`][embrace-operator] which calls `enquo()` implicitly):
...
For more information about external defusing, see [Taking multiple columns without ...][topic-multiple-columns].
